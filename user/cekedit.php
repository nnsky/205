     ...
     echo '<script>alert("Berhasil")</script>
		...
		if($array["status"]=="Sudah Digunakan" && $array["statvoucher"]=="Aktif"){
			...
			echo '<script>alert("GAGAL... NO SERI Tidak Dapat Digunakan")</script>
	         <script>setTimeout(function(){location.href="voucherck"},1)</script>';
		}elseif($array["status"]=="Belum Digunakan" && $array["statvoucher"]=="Aktif" && $array["lokasi"]==$lokasi){
			$serib=$seri;
			$lokasi=$array["lokasi"];
...
$update="update quote set tanggal='$tanggal',quote='$quote',customer='$customer',nama='$nama',perusahaan='$perusahaan',alamat='$alamat',pos='$pos',telp='$telp',ppn='$ppn',desk='$fstringdesk',jumlah='$fstringjumlah',harga='$fstringharga',diskon='$diskon',exp='$exp',berlaku='$berlaku',user='$user',note='$note',term='$stringterm',bank='$bank' where id='$id'";
$conn->query($update);
