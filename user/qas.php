...
if( !empty($requestData['search']['value']) ) {
	...
	$sql = "SELECT *";
	$sql.=" FROM asales";
	$sql.=" WHERE (tglkonfrim LIKE '%".$requestData['search']['value']."%' && lokasi ='".$_SESSION['lokasi']."')";  // $requestData['search']['value'] contains search parameter
	$sql.=" OR (ticketid LIKE '%".$requestData['search']['value']."%' && lokasi ='".$_SESSION['lokasi']."')";
	...
	$sql = "SELECT *";
	$sql.=" FROM asales where lokasi='".$_SESSION['lokasi']."'" ;
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	if($row["status"]=="pending"){
			...
			$nestedData[]="<a class='btn btn-success' href='tasrespon?&_id=".$row['ticketid']."&user=".$_SESSION['username']."'>Proses</a>";
	}else{
