<?php include "../koneksiremote.php"; $nama=$_POST['nama']; $nota=$_POST['nota']; $cabang=$_POST['cabang']; $kupon=strtoupper($_POST['kupon']); date_default_timezone_set('asia/jakarta'); $cek=mysqli_query($conn,"select *from 
newflash where kupon_aktif='$kupon'");
 if($cek->num_rows<=0){
    ...
    if($cek1['stock']==0){
      ...
      $insertpakai=$conn->query("insert into pakai (nama,email,kode_kupon,cabang,date,jenis) values ('$nama','$nota','$kupon','$cabang','$date','$jenis')");
       $updatekupon=$conn->query("update newflash set pakai='$pakai',kupon_aktif='$kodekupon',stock='$stock' where id='$id'");
